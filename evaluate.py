...
dataset_type = 'full' if args.use_full else '2k'
...
print(f"Evaluating only training: {eval_training}")
print(f"Do grouping first: {do_grouping_first}")

resultdir = 'Result-group-first' if do_grouping_first else 'Result-group-later'
...
logdir = Path('/local/home/enan/projects/loghub-2.0/{}_dataset'.format(dataset_type))
for dataset, setting in benchmark_settings.items():
    ...
    if eval_training: model.load_state_dict(torch.load(f'torch_model/model_{dataset}_{dataset_type}'))
    start=datetime.datetime.now()
    parse = preprocess.format_log(
        log_format=setting['log_format'],
        indir = str(logdir / dataset),
    )
    form = parse.format(f"{dataset}_{dataset_type}.log")
    ...
    form.to_csv(f'{resultdir}/{dataset}_{dataset_type}.log_structured.csv')
